...
<form class="edit_page" action="student_pass.php" method="POST">
			...
			<button class="reg_but button1" type="button" onclick="window.location.href='index.php'" name="sign_sub">Go to Dashboard</button><br><br>
...
<?php
	if(isset($_POST['sign_sub']))
	{
		...
		$g = $_SESSION['reg_number'];

		if(!empty($currentpass)){
			if(!empty($newpass)){
				if(!empty($confpass)){

					$loginsql="SELECT * FROM std_pass WHERE password='".md5($currentpass)."'";
					$result=mysqli_query($con,$loginsql) or mysqli_errno();
					$rows=mysqli_fetch_assoc($result);

					$a=$rows['password'];

					$query5="SELECT password FROM std_pass WHERE reg_number='".$g."'";
					...
					{
						if($newpass==$confpass){
							$query3="SELECT * FROM std_pass WHERE password='$newpass'";
							...
							else
							{
								...
								{
									$query2="UPDATE std_pass SET password='".md5($newpass)."' WHERE reg_number='".$g."'";
