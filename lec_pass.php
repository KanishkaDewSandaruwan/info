...
<form class="edit_page" action="lec_pass.php" method="POST">
			...
			<button class="reg_but button1" type="button" onclick="window.location.href='index2.php'" name="sign_sub">Go to Dashboard</button><br><br>
...
<?php
	if(isset($_POST['sign_sub']))
	{
		...
		$g = $_SESSION['lec_username'];

		if(!empty($currentpass)){
			if(!empty($newpass)){
				if(!empty($confpass)){

					$loginsql="SELECT * FROM lecture WHERE password='".md5($currentpass)."'";
					$result=mysqli_query($con,$loginsql) or mysqli_errno();
					$rows=mysqli_fetch_assoc($result);

					$a=$rows['password'];

					$query5="SELECT password FROM lecture WHERE lec_username='".$g."'";
					...
					{
						if($newpass==$confpass){
							$query3="SELECT * FROM lecture WHERE password='$newpass'";
							...
							else
							{
								...
								{
									$query2="UPDATE lecture SET password='".md5($newpass)."' WHERE lec_username='".$g."'";
